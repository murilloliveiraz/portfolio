  ...
  return (
      ...
      {data ? (
          data.projects.map((project) => (
            <article className="relative overflow-hidden rounded-3xl border-[1px] border-light-border bg-onyx-bg p-6 grid grid-cols-2 w-8/12 h-[565px]" key={project.projectTitle}>
                <div className="h-full flex flex-col justify-around">
                    <div className="flex flex-col gap-4">
                        <img src={project.logo} alt="logo do projeto" className="w-32"/>
                        <h1 className="text-5xl font-bold">{project.projectTitle}</h1>
                        <p className="text-gray-400 text-justify">{project.projectDescription}</p>
                    </div>
                ...
                <picture className="flex justify-center">
                  <img src={project.image} alt="imagem do projeto" className="w-[330px] rotate-6 object-contain"/>
                </picture>
            </article>
        ))
        ) : (
